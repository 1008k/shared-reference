...
function Resolve-ReadLockPath([string]$ProjectRoot, [string]$RequestedLockPath) {
  ...
  $newPath = Join-Path $ProjectRoot ".shared/shared-reference.lock.yaml"
  $oldSharedPath = Join-Path $ProjectRoot ".shared/agent-rules.lock.yaml"
  $oldPath = Join-Path $ProjectRoot ".agents/shared-rules.lock.yaml"
  if (Test-Path $newPath) { return $newPath }
  if (Test-Path $oldSharedPath) { return $oldSharedPath }
...
if (!$SharedRepo) { $SharedRepo = Join-Path (Split-Path $ProjectRoot -Parent) "shared-reference" }
...
$tempWorktree = $null
if ($Ref) {
  $tempWorktree = Join-Path ([IO.Path]::GetTempPath()) ("shared-reference-" + [guid]::NewGuid().ToString("N"))
...
version: 1
source:
  repo: https://github.com/1008k/shared-reference.git
  local_path: ../shared-reference
  ref: $resolvedRef
$managedBlock
$disabledBlock
...
"@ | Set-Content -Encoding UTF8 -NoNewline $LockPath
  ...
  Write-Output "shared-reference sync complete: $resolvedRef"
} finally {
