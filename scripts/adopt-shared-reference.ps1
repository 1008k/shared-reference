...
$relativeSharedRepo = "../shared-reference"
...
Write-ProjectWrapper (Join-Path $ProjectRoot "scripts/sync-shared-reference.ps1") $syncWrapper
Write-ProjectWrapper (Join-Path $ProjectRoot "scripts/propose-shared-reference-change.ps1") $proposeWrapper
Write-ProjectWrapper (Join-Path $ProjectRoot ".shared/shared-reference.lock.yaml") $lock

if ($DryRun) {
  Write-Output "Would sync managed files from $resolvedRef"
} else {
  & (Join-Path $SharedRepo "scripts/sync-shared-reference.ps1") -ProjectRoot $ProjectRoot -SharedRepo $SharedRepo -Ref $resolvedRef -Force
}

Write-Output "shared-reference adoption complete: $resolvedRef"
